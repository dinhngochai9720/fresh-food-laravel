                                    ...
                                    <div class="wsus__chatlist d-flex align-items-start">
                                        <div class="nav flex-column nav-pills me-3" id="v-pills-tab" role="tablist"
                                            ...
                                            <div class="wsus__chatlist_body">
                                                @foreach ($receivers_info as $receiver)
                                                    @php
                                                        // check receiver seen message or not
                                                        $unseen_messages = \App\Models\Message::where(['sender_id' => $receiver->receiverProfile->id, 'receiver_id' => auth()->user()->id, 'seen' => 0])->exists();
                                                    @endphp

                                                    <button class="nav-link chat-receiver-profile" id="seller-list-6"
                                                        data-id="{{ $receiver->receiverProfile->id }}" data-bs-toggle="pill"
                                                        data-bs-target="#v-pills-home" type="button" role="tab"
                                                        aria-controls="v-pills-home" aria-selected="true">

                                                        <div
                                                            class="wsus_chat_list_img {{ $unseen_messages ? 'msg-notification' : '' }}">
                                                            <img src="{{ isset($receiver->receiverProfile->vendor->banner) ? asset($receiver->receiverProfile->vendor->banner) : asset($receiver->receiverProfile->image) }}"
                                                                alt="user" class="img-fluid">
                                                        </div>
                                                        <div class="wsus_chat_list_text">
                                                            <h4>{{ isset($receiver->receiverProfile->vendor->shop_name) ? $receiver->receiverProfile->vendor->shop_name : $receiver->receiverProfile->name }}
                                                            </h4>
                                                        </div>
                                                    </button>
                                                @endforeach
                                                ...
                                                <div id="chat_box">
                                                    <div class="wsus__chat_area"
                                                        ...
                                                        <div class="wsus__chat_area_header">
                                                            <h2 id="name_receiver"></h2>
                                                        </div>
                                                        <div class="wsus__chat_area_body chat_body_dashboard_user"
                                                            data-inbox="">
                                                        ...
                                                        <div class="wsus__chat_area_footer"
                                                            ...
                                                            <form class="message_form_dashboard_user">
                                                                @csrf
...
@push('scripts')
    <script>
        const chat_are_body = $('.chat_body_dashboard_user');
        ...
        $(document).ready(function() {
            $('.chat-receiver-profile').on('click', function() {
                let receiver_id = $(this).data('id');

                let name_receiver = $(this).find('h4').text();

                var receiverImage = $(this).find('img').attr('src');

                $('#receiver_id').val(receiver_id);

                chat_are_body.attr('data-inbox', receiver_id);

                $(this).find(".wsus_chat_list_img").removeClass("msg-notification");

                $.ajax({
                    method: 'GET',
                    url: "{{ route('user.get-message') }}",
                    data: {
                        receiver_id: receiver_id,
                    },
                    beforeSend: function() {
                        // refresh chat content after choose new receiver
                        chat_are_body.html('');

                        $('#name_receiver').text(`${name_receiver}`);
                    },
            ...
            $('.message_form_dashboard_user').on('submit', function(e) {
                e.preventDefault();
