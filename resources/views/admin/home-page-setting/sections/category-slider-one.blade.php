@php
    $category_slider_one = json_decode($category_slider_one->value);
    // get sub categories of category
    $sub_categories = \App\Models\SubCategory::where('category_id', $category_slider_one[0]->category)->get();
    // get child categories of sub_category
    $child_categories = \App\Models\ChildCategory::where('sub_category_id', $category_slider_one[0]->sub_category)->get();
@endphp


<div class="tab-pane fade" id="category-slider-one-setting" role="tabpanel" aria-labelledby="category-slider-one-settings">
    <div class="card border">
        <div class="card-body">
            <form action="{{ route('admin.home-page-setting.category-slider-one.update') }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select id="category" class="form-control main-category" name="category">
                                ...
                                @foreach ($categories as $key => $category)
                                    <option {{ $category->id == $category_slider_one[0]->category ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <select id="sub_category" class="form-control sub-category" name="sub_category">
                                ...
                                @foreach ($sub_categories as $sub_category)
                                    <option
                                        {{ $sub_category->id == $category_slider_one[0]->sub_category ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <select id="child_category" class="form-control child-category" name="child_category">
                                ...
                                @foreach ($child_categories as $child_category)
                                    <option
                                        {{ $child_category->id == $category_slider_one[0]->child_category ? 'selected' : '' }}
