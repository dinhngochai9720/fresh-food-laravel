...
class ProductVariantItemController extends Controller
{
    ...
    public function index($product_id, $variant_id)
    {
        ...
        return view('vendor.product.variant-item.index', compact('product', 'variant'));
    }
    ...
    public function edit(string $variant_item_id)
    {
        ...
        if ($variant_item->variant->product->vendor_id !== Auth::user()->vendor->id) {
            abort(404);
        }

        return view('vendor.product.variant-item.edit', compact('variant_item'));
    }
    ...
    public function update(Request $request, string $variant_item_id)
    {
        ...
        // check product is not vendor created -> do no update
        if ($variant_item->variant->product->vendor_id !== Auth::user()->vendor->id) {
            abort(404);
        }
        ...
        return redirect()->route('vendor.product-variant-item.index', ["product_id" => $variant_item->variant->product_id, "variant_id" => $variant_item->variant_id]);
    }
    ...
    public function destroy(string $variant_item_id)
    {
        ...
        // check product is not vendor created -> do no destroy
        if ($variant_item->variant->product->vendor_id !== Auth::user()->vendor->id) {
            abort(404);
        }
